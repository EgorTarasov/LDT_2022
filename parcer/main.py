...
from app.sql_app import models, database
...
arr = []
with open('postv3.csv', 'w+', encoding="utf-8") as csvfile:
    ...
    while len(arr) < 50:
        ...
        for el in results:
            if len(el.text.split("\n")) == 9:
                ...
                lat, long = map(lambda x: float(x), coord.get_attribute("data-coordinates").split(","))
